.class public Lisn;
.super Lisl;


# static fields
.field private static l:Ljava/lang/Object;

.field private static volatile n:Litp;

.field private static o:Z

.field private static p:J


# instance fields
.field public m:Z

.field private q:Ljava/lang/String;


# direct methods
.method static constructor <clinit>()V
    .locals 2

    new-instance v0, Ljava/lang/Object;

    invoke-direct {v0}, Ljava/lang/Object;-><init>()V

    sput-object v0, Lisn;->l:Ljava/lang/Object;

    const-class v0, Lisn;

    invoke-virtual {v0}, Ljava/lang/Class;->getSimpleName()Ljava/lang/String;

    const/4 v0, 0x0

    sput-object v0, Lisn;->n:Litp;

    const/4 v0, 0x0

    sput-boolean v0, Lisn;->o:Z

    const-wide/16 v0, 0x0

    sput-wide v0, Lisn;->p:J

    return-void
.end method

.method protected constructor <init>(Landroid/content/Context;Ljava/lang/String;)V
    .locals 1

    const/4 v0, 0x0

    invoke-direct {p0, p1}, Lisl;-><init>(Landroid/content/Context;)V

    iput-boolean v0, p0, Lisn;->m:Z

    iput-object p2, p0, Lisn;->q:Ljava/lang/String;

    iput-boolean v0, p0, Lisn;->m:Z

    return-void
.end method

.method protected constructor <init>(Landroid/content/Context;Ljava/lang/String;Z)V
    .locals 1

    invoke-direct {p0, p1}, Lisl;-><init>(Landroid/content/Context;)V

    const/4 v0, 0x0

    iput-boolean v0, p0, Lisn;->m:Z

    iput-object p2, p0, Lisn;->q:Ljava/lang/String;

    iput-boolean p3, p0, Lisn;->m:Z

    return-void
.end method

.method private static a(Litp;Landroid/view/MotionEvent;Landroid/util/DisplayMetrics;)Litt;
    .locals 5

    .prologue
    .line 40000
    const-string v0, "o0GzQ1AM610CirJ0mcCKJHj/NnrALV7IVJAJSvbqxeR2lO5qr3ONp8jyJ7viBSRN"

    .line 41000
    const-string v1, "FA/E39LeBI2eD3MoCdtyeD9z2RehK7+vq57hni6whew="

    .line 0
    invoke-virtual {p0, v0, v1}, Litp;->a(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/reflect/Method;

    move-result-object v0

    if-eqz v0, :cond_0

    if-nez p1, :cond_1

    :cond_0
    new-instance v0, Lisu;

    invoke-direct {v0}, Lisu;-><init>()V

    throw v0

    :cond_1
    :try_start_0
    new-instance v1, Litt;

    const/4 v2, 0x0

    const/4 v3, 0x2

    new-array v3, v3, [Ljava/lang/Object;

    const/4 v4, 0x0

    aput-object p1, v3, v4

    const/4 v4, 0x1

    aput-object p2, v3, v4

    invoke-virtual {v0, v2, v3}, Ljava/lang/reflect/Method;->invoke(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/String;

    invoke-direct {v1, v0}, Litt;-><init>(Ljava/lang/String;)V
    :try_end_0
    .catch Ljava/lang/IllegalAccessException; {:try_start_0 .. :try_end_0} :catch_0
    .catch Ljava/lang/reflect/InvocationTargetException; {:try_start_0 .. :try_end_0} :catch_1

    return-object v1

    :catch_0
    move-exception v0

    :goto_0
    new-instance v1, Lisu;

    invoke-direct {v1, v0}, Lisu;-><init>(Ljava/lang/Throwable;)V

    throw v1

    :catch_1
    move-exception v0

    goto :goto_0
.end method

.method protected static declared-synchronized a(Landroid/content/Context;Z)V
    .locals 6

    const-class v1, Lisn;

    monitor-enter v1

    :try_start_0
    sget-boolean v0, Lisn;->o:Z

    if-nez v0, :cond_0

    invoke-static {}, Litu;->a()Ljava/lang/Long;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/Long;->longValue()J

    move-result-wide v2

    const-wide/16 v4, 0x3e8

    div-long/2addr v2, v4

    sput-wide v2, Lisn;->p:J

    invoke-static {p0, p1}, Lisn;->b(Landroid/content/Context;Z)Litp;

    move-result-object v0

    sput-object v0, Lisn;->n:Litp;

    const/4 v0, 0x1

    sput-boolean v0, Lisn;->o:Z
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    :cond_0
    monitor-exit v1

    return-void

    :catchall_0
    move-exception v0

    monitor-exit v1

    throw v0
.end method

.method protected static a(Ljava/util/List;)V
    .locals 4

    .prologue
    .line 0
    sget-object v0, Lisn;->n:Litp;

    if-nez v0, :cond_1

    :cond_0
    :goto_0
    return-void

    :cond_1
    sget-object v0, Lisn;->n:Litp;

    .line 51042
    iget-object v1, v0, Litp;->c:Ljava/util/concurrent/ExecutorService;

    .line 0
    if-eqz v1, :cond_0

    invoke-interface {p0}, Ljava/util/List;->isEmpty()Z

    move-result v0

    if-nez v0, :cond_0

    :try_start_0
    sget-object v0, Lhyt;->f:Lhyn;

    invoke-virtual {v0}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Long;

    invoke-virtual {v0}, Ljava/lang/Long;->longValue()J

    move-result-wide v2

    sget-object v0, Ljava/util/concurrent/TimeUnit;->MILLISECONDS:Ljava/util/concurrent/TimeUnit;

    invoke-interface {v1, p0, v2, v3, v0}, Ljava/util/concurrent/ExecutorService;->invokeAll(Ljava/util/Collection;JLjava/util/concurrent/TimeUnit;)Ljava/util/List;
    :try_end_0
    .catch Ljava/lang/InterruptedException; {:try_start_0 .. :try_end_0} :catch_0

    goto :goto_0

    :catch_0
    move-exception v0

    const-string v1, "class methods got exception: %s"

    const/4 v2, 0x1

    new-array v2, v2, [Ljava/lang/Object;

    const/4 v3, 0x0

    invoke-static {v0}, Litu;->a(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v0

    aput-object v0, v2, v3

    invoke-static {v1, v2}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    goto :goto_0
.end method

.method private static b(Landroid/content/Context;Z)Litp;
    .locals 5

    .prologue
    .line 0
    sget-object v0, Lisn;->n:Litp;

    if-nez v0, :cond_1

    sget-object v1, Lisn;->l:Ljava/lang/Object;

    monitor-enter v1

    :try_start_0
    sget-object v0, Lisn;->n:Litp;

    if-nez v0, :cond_0

    .line 1000
    const-string v0, "t2PZ/+5cxs0OItzPKdPmkcFYX6IMfjTFHkZNA+hRNgo="

    .line 2000
    const-string v2, ""

    .line 0
    invoke-static {p0, v0, v2, p1}, Litp;->a(Landroid/content/Context;Ljava/lang/String;Ljava/lang/String;Z)Litp;

    move-result-object v0

    .line 3000
    const-class v2, Landroid/content/Context;

    invoke-static {v2}, Ljava/util/Collections;->singletonList(Ljava/lang/Object;)Ljava/util/List;

    move-result-object v2

    .line 4000
    const-string v3, "VX9MrvHztGVmqQTTWxSZkJ73owIVOtYaZchCfsNMvtUeXKtUxi0Qcz+KKQxI08bI"

    .line 5000
    const-string v4, "JKwr/jWQAGiOfzioCYTXagRIrXFCbaU5iQDToiIZWP8="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 6000
    const-string v3, "SZRChvFBiLD4zy3ItGL8K1AQ5hOCgtqWEqCyd3cX1BuLXRwpVYP5SCW5cQd9XKS1"

    .line 7000
    const-string v4, "z+LKTwmnOGojuVGtNsel09o3LVPFBikDS/J5YQi6K04="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 8000
    const-string v3, "fmYaOjpn+lN4d93E2Wu9x7T14hH1fz7q9DblNdJC9JAHyMyGRRDs11r61WyjTUlM"

    .line 9000
    const-string v4, "CLoNd/n8AKRL2YLRZeSKpe/OPvyZCDqMnLEGlyL2Jf8="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 10000
    const-string v3, "4/anud6JYVZHYPnpt8BSjGAgN5OxmVcATlityGx7DjQ6zycLIOxxs7Yl66sa2K8i"

    .line 11000
    const-string v4, "jGlxJ8pjBX04ukIIrGJNFb6EFRVsGzYK7bIYQcZWtIw="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 12000
    const-string v3, "R6JE66TwM9X9zvPA5A2bEe7L3EFElBokkFmJUqhWRsQD1xENOXGSfBWjSfsRdHcc"

    .line 13000
    const-string v4, "oF+JuG1/2UpqTbYgc+zDqGftFlXBy6Rtf7wmF8IiM1g="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 14000
    const-string v3, "Uqy5Rvq9dq3haIK+moGj3h9af1gijPl2Uta8I1KZrsSADi0FXA2O30F2PPGTboqD"

    .line 15000
    const-string v4, "TVtXfU6OVDeQ88SzH0kWbapIsMLRv+u+D9k8ly0BLPg="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 16000
    const-string v3, "BnSC7c4mZmgqS1EgfjYxNp2AwFdvofFZS2zpMhDEJ9+6Ihx6R+CCB+bKEmf1eTYC"

    .line 17000
    const-string v4, "9Jo8EpxT6qvv5IsOaUBZjs5nVkxjW+lgFtp6J1MN2vI="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    const/4 v2, 0x2

    new-array v2, v2, [Ljava/lang/Class;

    const/4 v3, 0x0

    const-class v4, Landroid/view/MotionEvent;

    aput-object v4, v2, v3

    const/4 v3, 0x1

    const-class v4, Landroid/util/DisplayMetrics;

    aput-object v4, v2, v3

    invoke-static {v2}, Ljava/util/Arrays;->asList([Ljava/lang/Object;)Ljava/util/List;

    move-result-object v2

    .line 18000
    const-string v3, "o0GzQ1AM610CirJ0mcCKJHj/NnrALV7IVJAJSvbqxeR2lO5qr3ONp8jyJ7viBSRN"

    .line 19000
    const-string v4, "FA/E39LeBI2eD3MoCdtyeD9z2RehK7+vq57hni6whew="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 20000
    const-string v3, "1A3De6rl6U3r5HD0WWE4W44SagbtJTOLm6qqsAUcebMeVvCx4enOJ/JkVAkcUu+y"

    .line 21000
    const-string v4, "SWcwPOQrHSBVUTswYzxLtEctCWJYPYUBIwvOvD2bU4U="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 22000
    const-string v2, "CV9RdUb0nvKo5niij2H9dv7gsnC3Z1KT9RqjCnYupaWP64z//PWhEciMbQXLWBiO"

    .line 23000
    const-string v3, "y5WQSPATbCYHdumAH5CwwKVH4taU15SD+NxQLrV8eHg="

    .line 3000
    invoke-static {}, Ljava/util/Collections;->emptyList()Ljava/util/List;

    move-result-object v4

    invoke-virtual {v0, v2, v3, v4}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 24000
    const-string v2, "kKT5NGB44lxyy3ZRKQncf4Bvpk4C2a+GYnXlrueCY33aqIbaCYE6aPZEcEzDXcZB"

    .line 25000
    const-string v3, "ju/ZfZYRN5+ll/UfEMLIgLpKrtKoymQsquOEtRVAdz4="

    .line 3000
    invoke-static {}, Ljava/util/Collections;->emptyList()Ljava/util/List;

    move-result-object v4

    invoke-virtual {v0, v2, v3, v4}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 26000
    const-string v2, "nuFSVeZD/8tIpiPjd4egXJz+c1j+LjP0yk+YtEopZZVRZlkc8OJ3sbc4nTUCQEyq"

    .line 27000
    const-string v3, "Dga7NZkCRMmPHeyXsbbZdSrRfAooAGqT9TNLSykUoBA="

    .line 3000
    invoke-static {}, Ljava/util/Collections;->emptyList()Ljava/util/List;

    move-result-object v4

    invoke-virtual {v0, v2, v3, v4}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 28000
    const-string v2, "rbGSviQA4HSFbUsClKEReKj+UHhME2pQ49rpgO6TbfAmT+Kor5KnnnotmHx6YWzn"

    .line 29000
    const-string v3, "s1h8s1fn16eq1Nfx5usfqhMcnIJwOD4mjwhT1xM9gZw="

    .line 3000
    invoke-static {}, Ljava/util/Collections;->emptyList()Ljava/util/List;

    move-result-object v4

    invoke-virtual {v0, v2, v3, v4}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 30000
    const-string v2, "h/Dzks38C60cCM683AroxZlujMZD1gQSo6NRjSASnipq/3XuJpYtiQM5uZWC9cSf"

    .line 31000
    const-string v3, "TJ4FE5spMQItmxL3c7JnAGKLOPIvWg0Z12SDIorbzKQ="

    .line 3000
    invoke-static {}, Ljava/util/Collections;->emptyList()Ljava/util/List;

    move-result-object v4

    invoke-virtual {v0, v2, v3, v4}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 32000
    const-string v2, "XPIt/MiBR6wezjvYVDharIu+u7wW+j3tOBA4wOn+fmqoc9YnrIuRDBXxdQslpX19"

    .line 33000
    const-string v3, "v35iV8zCWl6VTh1YUpGi7vAwlEaUiupSCSZATHLI48w="

    .line 3000
    invoke-static {}, Ljava/util/Collections;->emptyList()Ljava/util/List;

    move-result-object v4

    invoke-virtual {v0, v2, v3, v4}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    const/4 v2, 0x3

    new-array v2, v2, [Ljava/lang/Class;

    const/4 v3, 0x0

    const-class v4, Landroid/content/Context;

    aput-object v4, v2, v3

    const/4 v3, 0x1

    sget-object v4, Ljava/lang/Boolean;->TYPE:Ljava/lang/Class;

    aput-object v4, v2, v3

    const/4 v3, 0x2

    sget-object v4, Ljava/lang/Boolean;->TYPE:Ljava/lang/Class;

    aput-object v4, v2, v3

    invoke-static {v2}, Ljava/util/Arrays;->asList([Ljava/lang/Object;)Ljava/util/List;

    move-result-object v2

    .line 34000
    const-string v3, "iPJixerRGyREGBs+8buE2PIilyjPvpe2jrcepbnc26mPMPeIbWCP9qil/Q/qntBL"

    .line 35000
    const-string v4, "MEwrz7/0++hBReCd/yzqiEqwd0MITddvuw69ATyPQDk="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    const/4 v2, 0x1

    new-array v2, v2, [Ljava/lang/Class;

    const/4 v3, 0x0

    const-class v4, [Ljava/lang/StackTraceElement;

    aput-object v4, v2, v3

    invoke-static {v2}, Ljava/util/Arrays;->asList([Ljava/lang/Object;)Ljava/util/List;

    move-result-object v2

    .line 36000
    const-string v3, "PkQbvO3Tufi9FeWwpxNipzQanokgfl9jPTi5LgFGxOf9IJukawkXXx+igTKy4F2N"

    .line 37000
    const-string v4, "rYpSxCYmzYyDoYtNchhULVKJd/P3Nq3ki5noTMVBsfo="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    const/4 v2, 0x1

    new-array v2, v2, [Ljava/lang/Class;

    const/4 v3, 0x0

    const-class v4, Landroid/view/View;

    aput-object v4, v2, v3

    invoke-static {v2}, Ljava/util/Arrays;->asList([Ljava/lang/Object;)Ljava/util/List;

    move-result-object v2

    .line 38000
    const-string v3, "wYuFOxx8cyqmp6JmCVltJ3JBpDlS8UcEFIVIRCgRolIeGKy/W40mSn4nON5NGKSw"

    .line 39000
    const-string v4, "XO4vVHo0eKEiPxlkyushGMBSBUwUHr1XAFo3YtsZ/UE="

    .line 3000
    invoke-virtual {v0, v3, v4, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;)Z

    .line 0
    sput-object v0, Lisn;->n:Litp;

    :cond_0
    monitor-exit v1
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    :cond_1
    sget-object v0, Lisn;->n:Litp;

    return-object v0

    :catchall_0
    move-exception v0

    :try_start_1
    monitor-exit v1
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    throw v0
.end method


# virtual methods
.method protected final a([Ljava/lang/StackTraceElement;)J
    .locals 5

    .prologue
    .line 0
    sget-object v0, Lisn;->n:Litp;

    .line 51045
    const-string v1, "PkQbvO3Tufi9FeWwpxNipzQanokgfl9jPTi5LgFGxOf9IJukawkXXx+igTKy4F2N"

    .line 51046
    const-string v2, "rYpSxCYmzYyDoYtNchhULVKJd/P3Nq3ki5noTMVBsfo="

    .line 0
    invoke-virtual {v0, v1, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/reflect/Method;

    move-result-object v0

    if-eqz v0, :cond_0

    if-nez p1, :cond_1

    :cond_0
    new-instance v0, Lisu;

    invoke-direct {v0}, Lisu;-><init>()V

    throw v0

    :cond_1
    :try_start_0
    new-instance v1, Litn;

    const/4 v2, 0x0

    const/4 v3, 0x1

    new-array v3, v3, [Ljava/lang/Object;

    const/4 v4, 0x0

    aput-object p1, v3, v4

    invoke-virtual {v0, v2, v3}, Ljava/lang/reflect/Method;->invoke(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/String;

    invoke-direct {v1, v0}, Litn;-><init>(Ljava/lang/String;)V

    iget-object v0, v1, Litn;->a:Ljava/lang/Long;

    invoke-virtual {v0}, Ljava/lang/Long;->longValue()J
    :try_end_0
    .catch Ljava/lang/IllegalAccessException; {:try_start_0 .. :try_end_0} :catch_0
    .catch Ljava/lang/reflect/InvocationTargetException; {:try_start_0 .. :try_end_0} :catch_1

    move-result-wide v0

    return-wide v0

    :catch_0
    move-exception v0

    :goto_0
    new-instance v1, Lisu;

    invoke-direct {v1, v0}, Lisu;-><init>(Ljava/lang/Throwable;)V

    throw v1

    :catch_1
    move-exception v0

    goto :goto_0
.end method

.method protected a(Landroid/content/Context;Landroid/view/View;)Liqu;
    .locals 18

    .prologue
    .line 0
    new-instance v6, Liqu;

    invoke-direct {v6}, Liqu;-><init>()V

    move-object/from16 v0, p0

    iget-object v2, v0, Lisn;->q:Ljava/lang/String;

    invoke-static {v2}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v2

    if-nez v2, :cond_0

    move-object/from16 v0, p0

    iget-object v2, v0, Lisn;->q:Ljava/lang/String;

    iput-object v2, v6, Liqu;->b:Ljava/lang/String;

    :cond_0
    move-object/from16 v0, p0

    iget-boolean v2, v0, Lisn;->m:Z

    move-object/from16 v0, p1

    invoke-static {v0, v2}, Lisn;->b(Landroid/content/Context;Z)Litp;

    move-result-object v3

    invoke-virtual {v3}, Litp;->c()V

    .line 43000
    :try_start_0
    move-object/from16 v0, p0

    iget-object v2, v0, Lisn;->a:Landroid/view/MotionEvent;

    move-object/from16 v0, p0

    iget-object v4, v0, Lisn;->k:Landroid/util/DisplayMetrics;

    invoke-static {v3, v2, v4}, Lisn;->a(Litp;Landroid/view/MotionEvent;Landroid/util/DisplayMetrics;)Litt;

    move-result-object v2

    iget-object v4, v2, Litt;->a:Ljava/lang/Long;

    iput-object v4, v6, Liqu;->g:Ljava/lang/Long;

    iget-object v4, v2, Litt;->b:Ljava/lang/Long;

    iput-object v4, v6, Liqu;->h:Ljava/lang/Long;

    iget-object v4, v2, Litt;->c:Ljava/lang/Long;

    iput-object v4, v6, Liqu;->i:Ljava/lang/Long;

    move-object/from16 v0, p0

    iget-boolean v4, v0, Lisn;->j:Z

    if-eqz v4, :cond_1

    iget-object v4, v2, Litt;->d:Ljava/lang/Long;

    iput-object v4, v6, Liqu;->u:Ljava/lang/Long;

    iget-object v2, v2, Litt;->e:Ljava/lang/Long;

    iput-object v2, v6, Liqu;->v:Ljava/lang/Long;

    :cond_1
    sget-object v2, Lhyt;->l:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-nez v2, :cond_2

    sget-object v2, Lhyt;->i:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-eqz v2, :cond_5

    :cond_2
    new-instance v4, Liqv;

    invoke-direct {v4}, Liqv;-><init>()V

    move-object/from16 v0, p0

    iget-object v2, v0, Lisn;->a:Landroid/view/MotionEvent;

    move-object/from16 v0, p0

    invoke-virtual {v0, v2}, Lisn;->b(Landroid/view/MotionEvent;)Litt;

    move-result-object v5

    iget-object v2, v5, Litt;->a:Ljava/lang/Long;

    iput-object v2, v4, Liqv;->a:Ljava/lang/Long;

    iget-object v2, v5, Litt;->b:Ljava/lang/Long;

    iput-object v2, v4, Liqv;->b:Ljava/lang/Long;

    iget-object v2, v5, Litt;->c:Ljava/lang/Long;

    iput-object v2, v4, Liqv;->h:Ljava/lang/Long;

    move-object/from16 v0, p0

    iget-boolean v2, v0, Lisn;->j:Z

    if-eqz v2, :cond_4

    iget-object v2, v5, Litt;->e:Ljava/lang/Long;

    iput-object v2, v4, Liqv;->c:Ljava/lang/Long;

    iget-object v2, v5, Litt;->d:Ljava/lang/Long;

    iput-object v2, v4, Liqv;->e:Ljava/lang/Long;

    iget-object v2, v5, Litt;->f:Ljava/lang/Long;

    invoke-virtual {v2}, Ljava/lang/Long;->longValue()J

    move-result-wide v8

    const-wide/16 v10, 0x0

    cmp-long v2, v8, v10

    if-eqz v2, :cond_a

    const/4 v2, 0x1

    :goto_0
    invoke-static {v2}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v2

    iput-object v2, v4, Liqv;->g:Ljava/lang/Integer;

    move-object/from16 v0, p0

    iget-wide v8, v0, Lisn;->d:J

    const-wide/16 v10, 0x0

    cmp-long v2, v8, v10

    if-lez v2, :cond_3

    move-object/from16 v0, p0

    iget-object v2, v0, Lisn;->k:Landroid/util/DisplayMetrics;

    if-eqz v2, :cond_b

    move-object/from16 v0, p0

    iget-wide v8, v0, Lisn;->i:J

    long-to-double v8, v8

    move-object/from16 v0, p0

    iget-wide v10, v0, Lisn;->d:J

    long-to-double v10, v10

    div-double/2addr v8, v10

    invoke-static {v8, v9}, Ljava/lang/Math;->round(D)J

    move-result-wide v8

    invoke-static {v8, v9}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    :goto_1
    iput-object v2, v4, Liqv;->d:Ljava/lang/Long;

    move-object/from16 v0, p0

    iget-wide v8, v0, Lisn;->h:J

    long-to-double v8, v8

    move-object/from16 v0, p0

    iget-wide v10, v0, Lisn;->d:J

    long-to-double v10, v10

    div-double/2addr v8, v10

    invoke-static {v8, v9}, Ljava/lang/Math;->round(D)J

    move-result-wide v8

    invoke-static {v8, v9}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    iput-object v2, v4, Liqv;->f:Ljava/lang/Long;

    :cond_3
    iget-object v2, v5, Litt;->i:Ljava/lang/Long;

    iput-object v2, v4, Liqv;->j:Ljava/lang/Long;

    iget-object v2, v5, Litt;->j:Ljava/lang/Long;

    iput-object v2, v4, Liqv;->i:Ljava/lang/Long;

    iget-object v2, v5, Litt;->k:Ljava/lang/Long;

    invoke-virtual {v2}, Ljava/lang/Long;->longValue()J

    move-result-wide v8

    const-wide/16 v10, 0x0

    cmp-long v2, v8, v10

    if-eqz v2, :cond_c

    const/4 v2, 0x1

    :goto_2
    invoke-static {v2}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v2

    iput-object v2, v4, Liqv;->k:Ljava/lang/Integer;

    move-object/from16 v0, p0

    iget-wide v8, v0, Lisn;->g:J

    const-wide/16 v10, 0x0

    cmp-long v2, v8, v10

    if-lez v2, :cond_4

    move-object/from16 v0, p0

    iget-wide v8, v0, Lisn;->g:J

    invoke-static {v8, v9}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    iput-object v2, v4, Liqv;->l:Ljava/lang/Long;

    :cond_4
    iput-object v4, v6, Liqu;->I:Liqv;
    :try_end_0
    .catch Lisu; {:try_start_0 .. :try_end_0} :catch_1

    :cond_5
    :goto_3
    move-object/from16 v0, p0

    iget-wide v4, v0, Lisn;->c:J

    const-wide/16 v8, 0x0

    cmp-long v2, v4, v8

    if-lez v2, :cond_6

    move-object/from16 v0, p0

    iget-wide v4, v0, Lisn;->c:J

    invoke-static {v4, v5}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    iput-object v2, v6, Liqu;->y:Ljava/lang/Long;

    :cond_6
    move-object/from16 v0, p0

    iget-wide v4, v0, Lisn;->d:J

    const-wide/16 v8, 0x0

    cmp-long v2, v4, v8

    if-lez v2, :cond_7

    move-object/from16 v0, p0

    iget-wide v4, v0, Lisn;->d:J

    invoke-static {v4, v5}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    iput-object v2, v6, Liqu;->x:Ljava/lang/Long;

    :cond_7
    move-object/from16 v0, p0

    iget-wide v4, v0, Lisn;->e:J

    const-wide/16 v8, 0x0

    cmp-long v2, v4, v8

    if-lez v2, :cond_8

    move-object/from16 v0, p0

    iget-wide v4, v0, Lisn;->e:J

    invoke-static {v4, v5}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    iput-object v2, v6, Liqu;->w:Ljava/lang/Long;

    :cond_8
    move-object/from16 v0, p0

    iget-wide v4, v0, Lisn;->f:J

    const-wide/16 v8, 0x0

    cmp-long v2, v4, v8

    if-lez v2, :cond_9

    move-object/from16 v0, p0

    iget-wide v4, v0, Lisn;->f:J

    invoke-static {v4, v5}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    iput-object v2, v6, Liqu;->z:Ljava/lang/Long;

    :cond_9
    :try_start_1
    move-object/from16 v0, p0

    iget-object v2, v0, Lisn;->b:Ljava/util/LinkedList;

    invoke-virtual {v2}, Ljava/util/LinkedList;->size()I

    move-result v2

    add-int/lit8 v5, v2, -0x1

    if-lez v5, :cond_d

    new-array v2, v5, [Liqv;

    iput-object v2, v6, Liqu;->J:[Liqv;

    const/4 v2, 0x0

    move v4, v2

    :goto_4
    if-ge v4, v5, :cond_d

    move-object/from16 v0, p0

    iget-object v2, v0, Lisn;->b:Ljava/util/LinkedList;

    invoke-virtual {v2, v4}, Ljava/util/LinkedList;->get(I)Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/view/MotionEvent;

    move-object/from16 v0, p0

    iget-object v7, v0, Lisn;->k:Landroid/util/DisplayMetrics;

    invoke-static {v3, v2, v7}, Lisn;->a(Litp;Landroid/view/MotionEvent;Landroid/util/DisplayMetrics;)Litt;

    move-result-object v2

    new-instance v7, Liqv;

    invoke-direct {v7}, Liqv;-><init>()V

    iget-object v8, v2, Litt;->a:Ljava/lang/Long;

    iput-object v8, v7, Liqv;->a:Ljava/lang/Long;

    iget-object v2, v2, Litt;->b:Ljava/lang/Long;

    iput-object v2, v7, Liqv;->b:Ljava/lang/Long;

    iget-object v2, v6, Liqu;->J:[Liqv;

    aput-object v7, v2, v4
    :try_end_1
    .catch Lisu; {:try_start_1 .. :try_end_1} :catch_0

    add-int/lit8 v2, v4, 0x1

    move v4, v2

    goto :goto_4

    :cond_a
    const/4 v2, 0x0

    goto/16 :goto_0

    :cond_b
    const/4 v2, 0x0

    goto/16 :goto_1

    :cond_c
    const/4 v2, 0x0

    goto/16 :goto_2

    :catch_0
    move-exception v2

    const/4 v2, 0x0

    iput-object v2, v6, Liqu;->J:[Liqv;

    .line 44000
    :cond_d
    new-instance v17, Ljava/util/ArrayList;

    invoke-direct/range {v17 .. v17}, Ljava/util/ArrayList;-><init>()V

    .line 45000
    iget-object v2, v3, Litp;->c:Ljava/util/concurrent/ExecutorService;

    .line 44000
    if-eqz v2, :cond_16

    invoke-virtual {v3}, Litp;->e()I

    move-result v7

    new-instance v2, Liud;

    invoke-direct {v2, v3, v6}, Liud;-><init>(Litp;Liqu;)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v2, Liug;

    .line 46000
    const-string v4, "nuFSVeZD/8tIpiPjd4egXJz+c1j+LjP0yk+YtEopZZVRZlkc8OJ3sbc4nTUCQEyq"

    .line 47000
    const-string v5, "Dga7NZkCRMmPHeyXsbbZdSrRfAooAGqT9TNLSykUoBA="

    .line 44000
    invoke-direct/range {v2 .. v7}, Liug;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v9, Liub;

    .line 48000
    const-string v11, "CV9RdUb0nvKo5niij2H9dv7gsnC3Z1KT9RqjCnYupaWP64z//PWhEciMbQXLWBiO"

    .line 49000
    const-string v12, "y5WQSPATbCYHdumAH5CwwKVH4taU15SD+NxQLrV8eHg="

    .line 44000
    sget-wide v14, Lisn;->p:J

    move-object v10, v3

    move-object v13, v6

    move/from16 v16, v7

    invoke-direct/range {v9 .. v16}, Liub;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;JI)V

    move-object/from16 v0, v17

    invoke-interface {v0, v9}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v2, Liua;

    .line 50000
    const-string v4, "rbGSviQA4HSFbUsClKEReKj+UHhME2pQ49rpgO6TbfAmT+Kor5KnnnotmHx6YWzn"

    .line 51000
    const-string v5, "s1h8s1fn16eq1Nfx5usfqhMcnIJwOD4mjwhT1xM9gZw="

    .line 44000
    invoke-direct/range {v2 .. v7}, Liua;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v2, Litw;

    .line 51001
    const-string v4, "Uqy5Rvq9dq3haIK+moGj3h9af1gijPl2Uta8I1KZrsSADi0FXA2O30F2PPGTboqD"

    .line 51002
    const-string v5, "TVtXfU6OVDeQ88SzH0kWbapIsMLRv+u+D9k8ly0BLPg="

    .line 44000
    invoke-direct/range {v2 .. v7}, Litw;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v2, Liue;

    .line 51003
    const-string v4, "h/Dzks38C60cCM683AroxZlujMZD1gQSo6NRjSASnipq/3XuJpYtiQM5uZWC9cSf"

    .line 51004
    const-string v5, "TJ4FE5spMQItmxL3c7JnAGKLOPIvWg0Z12SDIorbzKQ="

    .line 44000
    invoke-direct/range {v2 .. v7}, Liue;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    sget-object v2, Lhyt;->o:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-nez v2, :cond_e

    sget-object v2, Lhyt;->i:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-eqz v2, :cond_f

    :cond_e
    new-instance v2, Litz;

    .line 51005
    const-string v4, "4/anud6JYVZHYPnpt8BSjGAgN5OxmVcATlityGx7DjQ6zycLIOxxs7Yl66sa2K8i"

    .line 51006
    const-string v5, "jGlxJ8pjBX04ukIIrGJNFb6EFRVsGzYK7bIYQcZWtIw="

    .line 44000
    invoke-direct/range {v2 .. v7}, Litz;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    :cond_f
    sget-object v2, Lhyt;->k:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-nez v2, :cond_10

    sget-object v2, Lhyt;->i:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-eqz v2, :cond_11

    :cond_10
    new-instance v2, Liul;

    .line 51007
    const-string v4, "BnSC7c4mZmgqS1EgfjYxNp2AwFdvofFZS2zpMhDEJ9+6Ihx6R+CCB+bKEmf1eTYC"

    .line 51008
    const-string v5, "9Jo8EpxT6qvv5IsOaUBZjs5nVkxjW+lgFtp6J1MN2vI="

    .line 44000
    invoke-direct/range {v2 .. v7}, Liul;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    :cond_11
    sget-object v2, Lhyt;->n:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-nez v2, :cond_12

    sget-object v2, Lhyt;->i:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-eqz v2, :cond_13

    :cond_12
    new-instance v2, Liuj;

    .line 51009
    const-string v4, "XPIt/MiBR6wezjvYVDharIu+u7wW+j3tOBA4wOn+fmqoc9YnrIuRDBXxdQslpX19"

    .line 51010
    const-string v5, "v35iV8zCWl6VTh1YUpGi7vAwlEaUiupSCSZATHLI48w="

    .line 44000
    invoke-direct/range {v2 .. v7}, Liuj;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    :cond_13
    sget-object v2, Lhyt;->p:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-nez v2, :cond_14

    sget-object v2, Lhyt;->i:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-eqz v2, :cond_15

    :cond_14
    new-instance v2, Liui;

    .line 51011
    const-string v4, "PkQbvO3Tufi9FeWwpxNipzQanokgfl9jPTi5LgFGxOf9IJukawkXXx+igTKy4F2N"

    .line 51012
    const-string v5, "rYpSxCYmzYyDoYtNchhULVKJd/P3Nq3ki5noTMVBsfo="

    .line 44000
    new-instance v8, Ljava/lang/Throwable;

    invoke-direct {v8}, Ljava/lang/Throwable;-><init>()V

    invoke-virtual {v8}, Ljava/lang/Throwable;->getStackTrace()[Ljava/lang/StackTraceElement;

    move-result-object v8

    invoke-direct/range {v2 .. v8}, Liui;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I[Ljava/lang/StackTraceElement;)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    :cond_15
    sget-object v2, Lhyt;->q:Lhyn;

    invoke-virtual {v2}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    if-eqz v2, :cond_16

    new-instance v2, Lium;

    .line 51013
    const-string v4, "wYuFOxx8cyqmp6JmCVltJ3JBpDlS8UcEFIVIRCgRolIeGKy/W40mSn4nON5NGKSw"

    .line 51014
    const-string v5, "XO4vVHo0eKEiPxlkyushGMBSBUwUHr1XAFo3YtsZ/UE="

    move-object/from16 v8, p2

    .line 44000
    invoke-direct/range {v2 .. v8}, Lium;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;ILandroid/view/View;)V

    move-object/from16 v0, v17

    invoke-interface {v0, v2}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 42000
    :cond_16
    invoke-static/range {v17 .. v17}, Lisn;->a(Ljava/util/List;)V

    .line 0
    invoke-virtual {v3}, Litp;->d()V

    return-object v6

    :catch_1
    move-exception v2

    goto/16 :goto_3
.end method

.method protected final a(Landroid/content/Context;Liqj;)Liqu;
    .locals 2

    new-instance v0, Liqu;

    invoke-direct {v0}, Liqu;-><init>()V

    iget-object v1, p0, Lisn;->q:Ljava/lang/String;

    invoke-static {v1}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v1

    if-nez v1, :cond_0

    iget-object v1, p0, Lisn;->q:Ljava/lang/String;

    iput-object v1, v0, Liqu;->b:Ljava/lang/String;

    :cond_0
    iget-boolean v1, p0, Lisn;->m:Z

    invoke-static {p1, v1}, Lisn;->b(Landroid/content/Context;Z)Litp;

    move-result-object v1

    invoke-virtual {v1}, Litp;->c()V

    invoke-virtual {p0, v1, v0, p2}, Lisn;->a(Litp;Liqu;Liqj;)V

    invoke-virtual {v1}, Litp;->d()V

    return-object v0
.end method

.method protected a(Litp;Liqu;Liqj;)V
    .locals 1

    .prologue
    .line 51041
    iget-object v0, p1, Litp;->c:Ljava/util/concurrent/ExecutorService;

    .line 0
    if-nez v0, :cond_0

    :goto_0
    return-void

    :cond_0
    invoke-virtual {p0, p1, p2, p3}, Lisn;->b(Litp;Liqu;Liqj;)Ljava/util/List;

    move-result-object v0

    invoke-static {v0}, Lisn;->a(Ljava/util/List;)V

    goto :goto_0
.end method

.method protected final b(Landroid/view/MotionEvent;)Litt;
    .locals 6

    .prologue
    .line 0
    sget-object v0, Lisn;->n:Litp;

    .line 51043
    const-string v1, "1A3De6rl6U3r5HD0WWE4W44SagbtJTOLm6qqsAUcebMeVvCx4enOJ/JkVAkcUu+y"

    .line 51044
    const-string v2, "SWcwPOQrHSBVUTswYzxLtEctCWJYPYUBIwvOvD2bU4U="

    .line 0
    invoke-virtual {v0, v1, v2}, Litp;->a(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/reflect/Method;

    move-result-object v0

    if-eqz v0, :cond_0

    if-nez p1, :cond_1

    :cond_0
    new-instance v0, Lisu;

    invoke-direct {v0}, Lisu;-><init>()V

    throw v0

    :cond_1
    :try_start_0
    new-instance v1, Litt;

    const/4 v2, 0x0

    const/4 v3, 0x2

    new-array v3, v3, [Ljava/lang/Object;

    const/4 v4, 0x0

    aput-object p1, v3, v4

    const/4 v4, 0x1

    iget-object v5, p0, Lisn;->k:Landroid/util/DisplayMetrics;

    aput-object v5, v3, v4

    invoke-virtual {v0, v2, v3}, Ljava/lang/reflect/Method;->invoke(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/String;

    invoke-direct {v1, v0}, Litt;-><init>(Ljava/lang/String;)V
    :try_end_0
    .catch Ljava/lang/IllegalAccessException; {:try_start_0 .. :try_end_0} :catch_0
    .catch Ljava/lang/reflect/InvocationTargetException; {:try_start_0 .. :try_end_0} :catch_1

    return-object v1

    :catch_0
    move-exception v0

    :goto_0
    new-instance v1, Lisu;

    invoke-direct {v1, v0}, Lisu;-><init>(Ljava/lang/Throwable;)V

    throw v1

    :catch_1
    move-exception v0

    goto :goto_0
.end method

.method protected b(Litp;Liqu;Liqj;)Ljava/util/List;
    .locals 16

    .prologue
    .line 0
    invoke-virtual/range {p1 .. p1}, Litp;->e()I

    move-result v5

    new-instance v15, Ljava/util/ArrayList;

    invoke-direct {v15}, Ljava/util/ArrayList;-><init>()V

    new-instance v0, Lity;

    .line 51015
    const-string v2, "iPJixerRGyREGBs+8buE2PIilyjPvpe2jrcepbnc26mPMPeIbWCP9qil/Q/qntBL"

    .line 51016
    const-string v3, "MEwrz7/0++hBReCd/yzqiEqwd0MITddvuw69ATyPQDk="

    .line 0
    sget-object v1, Lhyt;->h:Lhyn;

    invoke-virtual {v1}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Ljava/lang/Boolean;

    invoke-virtual {v1}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v1

    if-nez v1, :cond_0

    sget-object v1, Lhyt;->i:Lhyn;

    invoke-virtual {v1}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Ljava/lang/Boolean;

    invoke-virtual {v1}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v1

    if-eqz v1, :cond_5

    :cond_0
    const/4 v6, 0x1

    :goto_0
    move-object/from16 v1, p1

    move-object/from16 v4, p2

    move-object/from16 v7, p3

    invoke-direct/range {v0 .. v7}, Lity;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;IZLiqj;)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v7, Liub;

    .line 51017
    const-string v9, "CV9RdUb0nvKo5niij2H9dv7gsnC3Z1KT9RqjCnYupaWP64z//PWhEciMbQXLWBiO"

    .line 51018
    const-string v10, "y5WQSPATbCYHdumAH5CwwKVH4taU15SD+NxQLrV8eHg="

    .line 0
    sget-wide v12, Lisn;->p:J

    move-object/from16 v8, p1

    move-object/from16 v11, p2

    move v14, v5

    invoke-direct/range {v7 .. v14}, Liub;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;JI)V

    invoke-interface {v15, v7}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Liug;

    .line 51019
    const-string v2, "nuFSVeZD/8tIpiPjd4egXJz+c1j+LjP0yk+YtEopZZVRZlkc8OJ3sbc4nTUCQEyq"

    .line 51020
    const-string v3, "Dga7NZkCRMmPHeyXsbbZdSrRfAooAGqT9TNLSykUoBA="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Liug;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Liuh;

    .line 51021
    const-string v2, "fmYaOjpn+lN4d93E2Wu9x7T14hH1fz7q9DblNdJC9JAHyMyGRRDs11r61WyjTUlM"

    .line 51022
    const-string v3, "CLoNd/n8AKRL2YLRZeSKpe/OPvyZCDqMnLEGlyL2Jf8="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Liuh;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Liuk;

    .line 51023
    const-string v2, "kKT5NGB44lxyy3ZRKQncf4Bvpk4C2a+GYnXlrueCY33aqIbaCYE6aPZEcEzDXcZB"

    .line 51024
    const-string v3, "ju/ZfZYRN5+ll/UfEMLIgLpKrtKoymQsquOEtRVAdz4="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Liuk;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Litx;

    .line 51025
    const-string v2, "SZRChvFBiLD4zy3ItGL8K1AQ5hOCgtqWEqCyd3cX1BuLXRwpVYP5SCW5cQd9XKS1"

    .line 51026
    const-string v3, "z+LKTwmnOGojuVGtNsel09o3LVPFBikDS/J5YQi6K04="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Litx;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Litz;

    .line 51027
    const-string v2, "4/anud6JYVZHYPnpt8BSjGAgN5OxmVcATlityGx7DjQ6zycLIOxxs7Yl66sa2K8i"

    .line 51028
    const-string v3, "jGlxJ8pjBX04ukIIrGJNFb6EFRVsGzYK7bIYQcZWtIw="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Litz;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Liuf;

    .line 51029
    const-string v2, "R6JE66TwM9X9zvPA5A2bEe7L3EFElBokkFmJUqhWRsQD1xENOXGSfBWjSfsRdHcc"

    .line 51030
    const-string v3, "oF+JuG1/2UpqTbYgc+zDqGftFlXBy6Rtf7wmF8IiM1g="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Liuf;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Litw;

    .line 51031
    const-string v2, "Uqy5Rvq9dq3haIK+moGj3h9af1gijPl2Uta8I1KZrsSADi0FXA2O30F2PPGTboqD"

    .line 51032
    const-string v3, "TVtXfU6OVDeQ88SzH0kWbapIsMLRv+u+D9k8ly0BLPg="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Litw;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Liua;

    .line 51033
    const-string v2, "rbGSviQA4HSFbUsClKEReKj+UHhME2pQ49rpgO6TbfAmT+Kor5KnnnotmHx6YWzn"

    .line 51034
    const-string v3, "s1h8s1fn16eq1Nfx5usfqhMcnIJwOD4mjwhT1xM9gZw="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Liua;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    new-instance v0, Liue;

    .line 51035
    const-string v2, "h/Dzks38C60cCM683AroxZlujMZD1gQSo6NRjSASnipq/3XuJpYtiQM5uZWC9cSf"

    .line 51036
    const-string v3, "TJ4FE5spMQItmxL3c7JnAGKLOPIvWg0Z12SDIorbzKQ="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Liue;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    sget-object v0, Lhyt;->j:Lhyn;

    invoke-virtual {v0}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Boolean;

    invoke-virtual {v0}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v0

    if-nez v0, :cond_1

    sget-object v0, Lhyt;->i:Lhyn;

    invoke-virtual {v0}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Boolean;

    invoke-virtual {v0}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v0

    if-eqz v0, :cond_2

    :cond_1
    new-instance v0, Liul;

    .line 51037
    const-string v2, "BnSC7c4mZmgqS1EgfjYxNp2AwFdvofFZS2zpMhDEJ9+6Ihx6R+CCB+bKEmf1eTYC"

    .line 51038
    const-string v3, "9Jo8EpxT6qvv5IsOaUBZjs5nVkxjW+lgFtp6J1MN2vI="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Liul;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    :cond_2
    sget-object v0, Lhyt;->m:Lhyn;

    invoke-virtual {v0}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Boolean;

    invoke-virtual {v0}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v0

    if-nez v0, :cond_3

    sget-object v0, Lhyt;->i:Lhyn;

    invoke-virtual {v0}, Lhyn;->a()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Boolean;

    invoke-virtual {v0}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v0

    if-eqz v0, :cond_4

    :cond_3
    new-instance v0, Liuj;

    .line 51039
    const-string v2, "XPIt/MiBR6wezjvYVDharIu+u7wW+j3tOBA4wOn+fmqoc9YnrIuRDBXxdQslpX19"

    .line 51040
    const-string v3, "v35iV8zCWl6VTh1YUpGi7vAwlEaUiupSCSZATHLI48w="

    move-object/from16 v1, p1

    move-object/from16 v4, p2

    .line 0
    invoke-direct/range {v0 .. v5}, Liuj;-><init>(Litp;Ljava/lang/String;Ljava/lang/String;Liqu;I)V

    invoke-interface {v15, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    :cond_4
    return-object v15

    :cond_5
    const/4 v6, 0x0

    goto/16 :goto_0
.end method
